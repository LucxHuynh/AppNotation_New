...
public partial class SignUpViewModel : ObservableObject
{
    ...
    private async Task RegisterUserAsync(string email, string password)
    {
        ...
        {
            StatusMessage = "Email này đã được sử dụng. Vui lòng sử dụng email khác hoặc đăng nhập.";
            await App.Current.MainPage.DisplayAlert("Lỗi", "Email này đã tồn tại", "OK");
        }
        catch (FirebaseAuthException ex)
        {
            StatusMessage = $"Đăng ký thất bại: {ex.Reason}";
            await App.Current.MainPage.DisplayAlert("Lỗi", StatusMessage, "OK");
        }
        catch (Exception ex)
        {
